...
Frozen snapshot of the board as inspected ~05:58–06:05 (host local). Do not flash a non-CameVision image. Do not swap kernels. Do not change USB gadget. Next work is part-by-part on top of this.

## Product
- Board: CameVision Single
- Processor: CameVision vision SoC, 1 GB DDR4, eMMC 14912 MB
- USB gadget: ADB only, `2207:0006`, strings Camemake / CameVision Single, serial `0558fa189447bc45`
- Windows may still show a generic ADB model string — that is adbd’s default, not another product

## What is running (live)
- Kernel: `6.1.141-rt52` `#24 SMP PREEMPT_RT` (2026-07-15)
- Hostname in rootfs is cosmetic only
...
Flash only with `restore-camevision.ps1` (CameVision boot).
...
| 0x7C40 | userdata | userdata.img (empty fs donor only) | b62ef1bd68294f64 |
