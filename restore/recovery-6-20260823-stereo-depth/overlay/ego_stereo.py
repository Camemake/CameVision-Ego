...
# Match on a 320x200 grid (exact 6x bin of 1920x1200), then nearest-upsample.
# 640x400 OpenCV BM is ~700ms on the RV1126B; 320x200 stays interactive.
CAP_FPS = 15
COLOR_PERIOD = 0.14
BW_FAST, BH_FAST, ND_FAST, TH_FAST = 320, 200, 48, 40
...
<div class="stage">
<div class="top">
 <div class="pane"><img class="v" src="/ov0"><div class="hud" id="h0">LEFT CAM0 IMU0</div></div>
 <div class="pane"><img class="v" src="/ov1"><div class="hud" id="h1">RIGHT CAM1 IMU1</div></div>
</div>
<div class="bot"><img class="v" src="/xyz"><div class="hud" id="hz">3D depth  75.000 mm  NPU</div></div>
</div>
<script>
...
async function t(){
 let d=null,s=null;
 try{ d=await (await fetch("http://127.0.0.1:8083/")).json(); }catch(e){}
 try{ s=await (await fetch("/stat")).json(); }catch(e){}
 if(d){
  h0.textContent=blk("LEFT   CAM0  IMU0",d.imu0,d,s);
  h1.textContent=blk("RIGHT  CAM1  IMU1",d.imu1,d,s);
 }
 if(s) hz.textContent="3D depth  "+((s&&s.xw)||1920)+"x"+((s&&s.xh)||1200)+"  "+n(s.heat_fps||s.depth_fps,1)+" fps  "+(s.kind||"")+" "+(s.grid||"")+"  matched "+n(s.fill,0)+"%  align "+(s.align_px||"")+" px"
  +(s.f_px?"  f "+n(s.f_px,0)+" px":"");
 setTimeout(t,40);
}
t();
</script></body></html>
...
def fit_align(left, right) -> str:
    ...
    ok = (p1[:, 0] - p2[:, 0] > 0.5) & (p1[:, 0] - p2[:, 0] < ND * 1.5)
    ok &= np.abs(p1[:, 1] - p2[:, 1]) < BH * 0.15
    p1, p2 = p1[ok], p2[ok]
    if len(p1) < 20:
    ...
    A = _quad_basis(P1[:, 0] / float(BW), P1[:, 1] / float(BH)).astype(np.float64)
    ...
    if best is None:
        if after > before * 0.7:
            return "no gain %.2f" % after
    elif after >= best:
        return "ok %.2f" % best
    X, Y = np.meshgrid(np.arange(BW, dtype=np.float32), np.arange(BH, dtype=np.float32))
    shift = _quad_basis(X / float(BW), Y / float(BH)) @ coef
    if float(np.abs(shift).max()) > BH * 0.2:
...
def _refine(left, right, d0, nd: int, span: int = 2, win: int = 5):
    """Search a few disparities around an estimate, at full grid resolution.

    This is what keeps detail affordable: the global pass runs on a half-size
    grid and only this local search touches every pixel. The right eye is
    warped by the estimate once, so each candidate is then a plain column
    offset rather than another per-pixel gather.
    """
    import cv2
    import numpy as np
    ...
    # A flat surface matches equally well at every candidate, so only move off
    # the estimate where the evidence is real. Otherwise noise would mottle it.
    gain = cv2.subtract(stack[span], cost)
    off = np.where(gain > win * win * 3, take - np.int16(span), np.int16(0))
    pick = np.clip(d0 + off, 0, nd - 1)
    # Where the right eye has no column to compare against, keep the estimate.
    good = (cost <= win * win * 20) | (grid[2] < pick)
...
def stereo_bm(L, R) -> bytearray:
    try:
        ...
        if _cv_ok():
            ...
            dc[raw < 16] = 0
            shared["pts"] = int(np.count_nonzero(dc)) * 4
            t_f = time.monotonic()
            dc = _densify(dc, ndc)
            d0 = cv2.resize(dc, (BW, BH), interpolation=cv2.INTER_LINEAR).astype(np.int16) * 2
            d = _refine(left, right, d0, ND)
            shared["ref_ms"] = (time.monotonic() - t_f) * 1000
            shared["hit"] = int(np.count_nonzero(d))
            t_f = time.monotonic()
            d = _densify(d)
            # Blend only where depth barely moved, so still surfaces stop
            # flickering while real motion stays sharp.
            prev = shared.get("_prev_d")
            if prev is not None and prev.shape == d.shape:
                calm = cv2.absdiff(d, prev) < 5
...
def render_xyz(disp, left=None) -> bytearray:
    ...
    try:
        ...
        if _cv_ok():
            ...
            big = cv2.resize(idx_s, (XW, XH), interpolation=cv2.INTER_LINEAR)
            half = cv2.resize(idx_s, (uw, uh), interpolation=cv2.INTER_LINEAR)
            y = cv2.LUT(big, lut_y)
