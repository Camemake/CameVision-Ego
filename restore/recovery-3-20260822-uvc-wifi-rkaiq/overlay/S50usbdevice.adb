...
configfs_init()
{
	...
	if [ -z $SERIAL -o "$SERIAL" = "0" ];then
		if [ -f "/sys/class/net/eth0/address" ];then
			SERIAL=$(cat /sys/class/net/eth0/address| md5sum)
		else
	...
	echo "CameMake"  > ${USB_STRINGS_DIR}/manufacturer
	echo "CameVision Ego"  > ${USB_STRINGS_DIR}/product
...
bind_functions()
{
	...
	if [ $UVC_EN = on ];then
		...
		ln -s ${UVC_CONTROL_DIR}/header/h ${UVC_CONTROL_DIR}/class/hs/h
