#!/usr/bin/env python3
"""M1 FIT, in-place bootargs only: root p9 -> p7, keep erofs.

Aura SPL/U-Boot stay in the chain. The M1 erofs system_a image is written to
Aura's rootfs slot (mmcblk0p7). Do not reserialize the DTB.
"""
...
DST = Path(r"C:\Users\stefa\Desktop\CameVision Single\build\camevision_boot_erofs.img")
BOOT_PART = 0x5800 * 512

# same-length replacements only
EDITS = [
    (b"root=/dev/mmcblk0p9", b"root=/dev/mmcblk0p7"),
    (b"pstore_blk.blkdev=/dev/mmcblk0p13", b"pstore_blk.blkdev=/dev/mmcblk0p7 "),
]
...
def main() -> int:
    ...
    if b"mmcblk0p9" in fdt:
        raise SystemExit("p9 still in fdt")
    if b"rootfstype=erofs" not in fdt:
        raise SystemExit("erofs missing from fdt")
    if b"rootfstype=ext4" in fdt:
        raise SystemExit("ext4 leaked into fdt")
