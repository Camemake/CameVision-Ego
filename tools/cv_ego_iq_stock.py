#!/usr/bin/env python3
"""Put the original linear IQ back and restart 3A. No HDR, no extra ISP blocks."""
import subprocess
import time
...
def main() -> int:
    ...
    sh(
        s,
        "python3 -c \"import json;d=json.load(open('/oem/usr/share/iqfiles/%s'));"
        "e=d['main_scene'][0]['sub_scene'][0]['scene_isp35'];"
        "print('iq ynr',e['ynr']['en'],'cnr',e['cnr']['en'],'sharp',e['sharp']['en'],"
        "'enh',e['enh']['en'],'drc',e['drc']['en'])\"; "
        "echo === hw ===; grep -E 'YNR|CNR|SHARP|ENH|GIC|CAC|DRC|LSC|CCM|GAMMA' /proc/rkisp-vir0; "
        "ps | grep rkaiq_3A | grep -v grep" % NAME,
    )
    return 0
